...
const {
  MessageEmbed, Modal, MessageActionRow, TextInputComponent,
} = require('discord.js');
...
function generateCaptchaString() {
  ...
  for (let i = 0; i < 6; i++) { str += `${letters[Math.floor(Math.random() * letters.length)]} `; }
  ...
  async execute(interaction) {
    ...
    interaction.awaitModalSubmit({ filter, time: 250_000 })
      .then(async (modal) => {
        ...
        const chan = interaction.guild.channels.cache.get('948680525235777576');
